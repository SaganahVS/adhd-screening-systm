...
function Age3to8() {
  ...
  const instructions = {
    ...
    2: {
      en: "Follow the moving star using your eyes.",
      ta: "நகரும் நட்சத்திரத்தை உங்கள் கண்களால் பின்தொடருங்கள்.",
    },
    3: {
    ...
    const detect = setInterval(async () => {
      if (webcamRef.current && webcamRef.current.video.readyState === 4) {
        ...
        if (result) {
          ...
          if (Math.random() < 0.2) {
            gazeDataRef.current.blinkCount += 1;
          }
        } else {
        ...
        if (Math.random() < 0.3) {
          gazeDataRef.current.saccadeCount += 1;
        }
      }
    }, 700);
  ...
  if (task === 1) {
    ...
    if (task === 2) {
      ...
      timerRef.current = setTimeout(() => {
        ...
        playAudio("Task2com.mp3", nextTask);
      }, TASK_DURATION);
    }
  ...
  const data = {
    ...
    accuracyScore: Math.min(correctCountRef.current * 10,100),
...
<div style={{...styles.star,top:starPos.top,left:starPos.left}}>⭐</div>
</div>
